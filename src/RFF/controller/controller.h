...
namespace FFS {
	...
	public:
		...
		template<typename ...modules_t, typename eventTags>
		Controller(std::tuple<FFS::Mode> _modes, eventTags event_tags, modules_t& ..._modules) : modes{_modes} {

			// From : https://stackoverflow.com/questions/62652638
			func = [this, event_tags, &_modules...](std::any && any_ev) mutable {      // invoked _modules' copy constructor which tries to copy the event handlers and the Tasks inside => fail.
				auto success = false;

				// Switch to generalized lambda capture from https://stackoverflow.com/questions/8640393
				auto f = [this, &any_ev, &_modules...](auto tag) -> bool {
